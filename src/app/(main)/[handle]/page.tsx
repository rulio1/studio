...
export default function ProfilePage() {
    ...
    const fetchUserReplies = useCallback(async (profileUid: string) => {
        if (!profileUid) return;
        setIsLoadingReplies(true);
        const q = query(collection(db, "comments"), where("authorId", "==", profileUid));
        ...
        const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
            ...
            const unsubscribeProfile = onSnapshot(q, async (querySnapshot) => {
                 if (querySnapshot.empty) {
                    setIsLoading(false);
                    setProfileUser(null); // No user found with this handle
                    return;
                }
    ...
    }, [profileHandle, router, fetchUserPosts, fetchUserReplies, fetchLikedPosts, toast]);
