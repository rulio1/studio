...
import { doc, getDoc, collection, query, where, getDocs, orderBy, updateDoc, arrayUnion, arrayRemove, onSnapshot, DocumentData, QuerySnapshot, writeBatch, serverTimestamp, deleteDoc, setDoc, documentId, addDoc, runTransaction, increment, limit } from 'firebase/firestore';
...
export default function ProfilePage() {
    ...
    useEffect(() => {
        if (searchParams.get('payment_success') === 'true') {
            ...
            router.replace(`/${handle}`, undefined);
        }
    }, [searchParams, toast, handle, router]);

    const fetchUserPosts = useCallback((userId: string, profileData: ZisprUser) => {
        ...
        const repostsQuery = query(collection(db, "reposts"), where("userId", "==", userId));
    ...
    const fetchUserReplies = useCallback(async (userId: string) => {
        if (!userId) return;
        setIsLoadingReplies(true);
        try {
            const q = query(collection(db, "comments"), where("authorId", "==", userId));
    ...
    const fetchLikedPosts = useCallback(async (userId: string, profileData: ZisprUser) => {
        if (profileData.likesArePrivate && profileData.uid !== currentUser?.uid) {
            setLikedPosts([]);
        ...
        try {
            ...
            const likedPostIds = userDoc.data()?.likes || [];
    ...
    useEffect(() => {
        ...
        const unsubscribeProfile = onSnapshot(q, async (snapshot) => {
            ...
            setIsFollowedBy(zisprUser?.followers?.includes(profileDoc.id) || false);

            if (!profileData.blockedBy?.includes(currentUser.uid)) {
                await fetchUserReplies(profileDoc.id);
                await fetchLikedPosts(profileDoc.id, profileData);
            } else {
                setUserPosts([]);
                setUserReplies([]);
                setMediaPosts([]);
    ...
    }, [handle, currentUser, zisprUser, fetchUserReplies, fetchLikedPosts, toast]);
    
    // New useEffect specifically for posts to handle real-time updates
    useEffect(() => {
        if (currentUser && profileUser && profileId && !hasBlockedYou) {
            const unsubscribe = fetchUserPosts(profileId, profileUser);
            return () => unsubscribe();
        }
    }, [currentUser, profileUser, profileId, hasBlockedYou, fetchUserPosts]);


    const handleToggleFollow = useCallback(async () => {
        if (!currentUser || !profileUser || !zisprUser || !profileId) return;
    ...
    }, [currentUser, profileUser, zisprUser, isFollowing, toast, profileId]);

    const handleStartConversation = useCallback(async () => {
        if (!currentUser || !profileUser || !profileId || currentUser.uid === profileId) return;

        const conversationId = [currentUser.uid, profileId].sort().join('_');
        ...
        try {
            ...
            if (!docSnap.exists()) {
                 await setDoc(conversationRef, {
                    ...
                    unreadCounts: { [currentUser.uid]: 0, [profileId]: 0 },
                    lastMessage: {
    ...
    }, [currentUser, profileUser, router, t, toast, profileId]);
                ...
                <div className="flex justify-between items-start">
                    ...
                    {isOwnProfile ? (
                        <Button variant="outline" className="rounded-full mt-4 font-bold" asChild>
                          <Link href={`/${handle}/edit`}>{t('profile.buttons.editProfile')}</Link>
                        </Button>
                    ) : (
